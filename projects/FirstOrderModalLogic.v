...
Module FirstOrderModalLogic.
  ...
  Definition get_ty_of_symbol (c : symbol) : ty :=
    match c with
  ...
  Inductive typeOf : tm -> ty -> Set :=
  | VAR_typeOf (x : ivar) : typeOf (VAR x) \ty( i )
  | CON_typeOf (c : symbol) : typeOf (CON c) (get_ty_of_symbol c)
  ...
  Arguments VAR_typeOf {lsig}.
  Arguments CON_typeOf {lsig}.
  Arguments APP_typeOf {lsig}.
  Arguments LAM_typeOf {lsig}.

  Global Notation " '\vdash_{' lsig  '}' tm_expr '\isof' ty_expr " := (typeOf lsig tm_expr ty_expr) (at level 70) : type_scope.
  ...
  Fixpoint interpreteW_tm (ivar_env : ivar -> wUniv) (tm_expr : tm lsig) (ty_expr : ty) (H : \vdash_{ lsig } tm_expr \isof ty_expr) {struct H} : interpreteW_ty ty_expr :=
    match H with
