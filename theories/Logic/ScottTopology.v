...
Module ScottTopology.
  ...
  | ScottTopologyOpen
    ...
    (LIMIT : forall X : ensemble D, << NONEMPTY : exists x0 : D, member x0 X >> -> << IS_DIRECTED : isDirectedSubset X >> -> forall sup_X : D, << IS_SUPREMUM : isSupremumOf sup_X X >> -> << SUPREMUM_IN : member sup_X O >> -> exists x : D, member x X /\ member x O)
    : isOpen_Scott O
  .
  ...
  Proof with (now vm_compute; firstorder) || (eauto with *).
    ...
    - ii; desnw in *; unnw. destruct X_isOpen as [? ?]. econstructor.
      ...
      + ii; desnw in *. unnw. exploit (LIMIT X0) as [? [? ?]]...
  Qed.
