...
Module FunFacts.
  ...
  Theorem classical_if_then_else (P : Prop) (exclusive_middle_P : P \/ ~ P) :
    {P} + {~ P}.
  Proof.
    ...
    apply axiom_schema_of_replacement_on_bool; destruct exclusive_middle_P as [H_yes | H_no]; [exists (true) | exists (false)]; now intros [ | ]; split.
  Qed.
  ...
  Proof.
    revert phi ACC_HYP.
    enough (it_is_sufficient_to_show : forall rank : nat, forall tree : A, get_rank tree < rank -> acc LT tree).
    enough (LT_well_founded : forall root : A, acc LT root).
    - exact (noetherian_recursion LT LT_well_founded).
    - intros root.
      apply it_is_sufficient_to_show with (rank := 1 + get_rank root).
