...
Module UntypedLamdbdaCalculus.
  ...
  Proof.
    set (my_eq_elim := fun L0 : tm => fun H : L0 = L => fun phi : forall L1 : tm, L1 = L -> Type => fun phi_pf : phi L (eq_reflexivity L) => @rect_eq_r tm L phi phi_pf L0 H).
    intros XP case_Refl case_AppL case_AppR case_LAbs.
    set (XP_Refl := fun M0 : tm => fun H : M0 = L => my_eq_elim M0 H (fun M1 : tm => fun H0 : M1 = L => XP M1 (eq_rect M1 (fun N1 : tm => subtm N1 M1) (subtmRefl M1) L H0)) case_Refl).
    set (XP_AppL := fun M0 : tm => fun P1 : tm => fun P2 : tm => fun H : M0 = L => my_eq_elim M0 H (fun M1 : tm => fun H0 : M1 = L => forall X' : subtm M1 P1, (forall H' : M1 = L, XP P1 (eq_rect M1 (fun N1 : tm => subtm N1 P1) X' L H')) -> XP (tmApp P1 P2) (eq_rect M1 (fun N1 : tm => subtm N1 (tmApp P1 P2)) (subtmAppL M1 P1 P2 X') L H0)) (fun X' : subtm L P1 => fun IHX' : forall H' : L = L, XP P1 (eq_rect L (fun N1 : tm => subtm N1 P1) X' L H') => case_AppL P1 P2 X' (IHX' (eq_reflexivity L)))).
    set (XP_AppR := fun M0 : tm => fun P1 : tm => fun P2 : tm => fun H : M0 = L => my_eq_elim M0 H (fun M1 : tm => fun H0 : M1 = L => forall X' : subtm M1 P2, (forall H' : M1 = L, XP P2 (eq_rect M1 (fun N1 : tm => subtm N1 P2) X' L H')) -> XP (tmApp P1 P2) (eq_rect M1 (fun N1 : tm => subtm N1 (tmApp P1 P2)) (subtmAppR M1 P1 P2 X') L H0)) (fun X' : subtm L P2 => fun IHX' : forall H' : L = L, XP P2 (eq_rect L (fun N1 : tm => subtm N1 P2) X' L H') => case_AppR P1 P2 X' (IHX' (eq_reflexivity L)))).
    set (XP_LAbs := fun M0 : tm => fun y : ivar => fun Q : tm => fun H : M0 = L => my_eq_elim M0 H (fun M1 : tm => fun H0 : M1 = L => forall X' : subtm M1 Q, (forall H' : M1 = L, XP Q (eq_rect M1 (fun N1 : tm => subtm N1 Q) X' L H')) -> XP (tmLam y Q) (eq_rect M1 (fun N1 : tm => subtm N1 (tmLam y Q)) (subtmLAbs M1 y Q X') L H0)) (fun X' : subtm L Q => fun IHX' : forall H' : L = L, XP Q (eq_rect L (fun N1 : tm => subtm N1 Q) X' L H') => case_LAbs y Q X' (IHX' (eq_reflexivity L)))).
    enough (go : forall N : tm, forall M : tm, forall X : subtm N M, forall H : N = L, XP M (eq_rect N (fun N0 : tm => subtm N0 M) X L H)) by exact (fun M : tm => fun X : subtm L M => go L M X (eq_reflexivity L)).
    exact (
