...
Module MyUtilities.
  ...
  Theorem divmod_unique (a : nat) b q r (H_b_ne_0 : b <> 0) :
    (a = b * q + r /\ r < b) <->
    (q = (a - r) / b /\ r = a mod b).
  Proof with lia || eauto.
    assert (lemma1 : forall x : nat, forall y : nat, x > y <-> (exists z : nat, x = S (y + z))).
    { split.
      - intros Hgt; induction Hgt as [ | m Hgt [z Heq]]; [exists 0 | rewrite Heq]...
      - intros [z Heq]...
    }
    split.
    - intros [H_a H_r_bound].
      ...
      enough (claim6 : q = a / b)...
      + split...
        replace (a - r) with (q * b)...
        symmetry; apply Nat.div_mul...
      + intros H_false.
        destruct (proj1 (lemma1 (a / b) q) H_false) as [x Hx]...
      + intros H_false.
        destruct (proj1 (lemma1 q (a / b)) H_false) as [x Hx]...
    - intros [H_q H_r].
      assert (claim1 := Nat.mod_bound_pos a b).
      split...
      assert (claim2 : r < b)...
      assert (claim3 := Nat.div_mod a b H_b_ne_0).
      rewrite <- H_r in claim3.
      enough (to_show : q = a / b)...
      rewrite H_q; symmetry.
