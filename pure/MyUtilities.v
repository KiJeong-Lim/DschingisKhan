...
Module MyUtilities.
  ...
  Proof with lia || eauto.
    assert (claim1 := div_mod_inv n_odd 2 n 1)...
  Qed.
  ...
  Proof with lia || eauto.
    assert (claim1 := div_mod_inv (n_even - 2) 2 n 0). split.
    - intros ?; subst.
  ...
  Definition Some_inj {A : Type} : forall x : A, forall y : A, Some x = Some y -> x = y :=
    fun x : A =>
    fun y : A =>
